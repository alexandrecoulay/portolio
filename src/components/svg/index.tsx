...
function SvgElement({ onClick, name, size = 33, margin, noColor, hover, pointer, className }: SectionType) {
    ...
    return (
        ...
        }} onClick={onClick ? onClick : undefined} className={`${noColor ? "" : styles.fa_secondary} ${hover ? styles.hover : ""} ${pointer ? styles.pointer : ""} ${className ? className : ""}` } xmlns="http://www.w3.org/2000/svg" viewBox={`0 0 ${svg.w} ${svg.h}`}>
            {
